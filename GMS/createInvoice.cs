...
namespace GMS
{
    public partial class createInvoice : Form
    {
        ...
        private void createInvoice_Load(object sender, EventArgs e)
        {
            ...
            cmbCat.Items.Add("Burgemen"); // add item to combo box
        ...
        private string getInvId()
        {
            ...
            string query = "SELECT TOP (1) invoiceID FROM invoice_Details ORDER BY invoiceID DESC";
            ...
            {
                dr.Read();
                string id = dr["invoiceID"].ToString(); //Largest Quote id in the table eg. QUO00016
        ...
        private void saveJob()
        {
            try
            {
                ...
                rds.Name = "DataSet1";
                rds.Value = lst;
                ReportInvoice frmp = new ReportInvoice();
                ...
                frmp.reportViewer2.LocalReport.ReportEmbeddedResource = "GMS.frmInvoiceReport.rdlc";



                frmp.reportViewer2.LocalReport.SetParameters(repParams);
                frmp.reportViewer2.RefreshReport();
                MessageBox.Show("Invoice Saved : " + lblQuoteId.Text + " Successfully inserted", "SUCCESS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                con.Close();
                frmp.ShowDialog();
                resetForm();
            }
